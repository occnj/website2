...
const NAV_FIELDS = [
  ...
  { key: 'href', label: 'Link', half: true, placeholder: 'about.html or https://…' },
];
function addNavItem() {
...
async function editPage(pageId) {
  ...
  try {
    ...
    const blockHtml = blocks.map(function (b, bi) {
      const fields = Object.entries(b.content).map(function (kv) {
        ...
        if (/image|photo/.test(k)) {
          return '<div class="form-group"><label class="form-label">' + esc(k) + '</label>' +
    ...
    container.innerHTML = '<div class="view active" data-screen-label="Page Editor">' +
      ...
      '<a class="btn btn-sm btn-outline" href="' + esc(page.slug) + '.html" target="_blank">Preview ↗</a>' +
